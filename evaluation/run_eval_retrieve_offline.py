...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The name of the retrieval database",
        required=True,
    )
    parser.add_argument(
        "--dataset_name",
...
def main():
    ...
    if "dpr" in args.prompt_mode:
        ...
        retrieve_num = 1
        concat_num = 1
        # retreived_dataset_root = "/cephfs/user/mikeeewang/summer_22/code/t-zero/evaluation/retrieved_dataset_validation/p3_subset_6_1"
        retreived_dataset_root = f"/cephfs/user/mikeeewang/summer_22/code/t-zero/evaluation/retrieved_dataset_train_validation/{args.shard_name}"
        # raw_datasets = load_from_disk(os.path.join(retreived_dataset_root, args.dataset_name))
    else:
        retrieve_num = 0
        concat_num = 0
    ...
    for template_name in template_names:
        ...
        if 'dpr' in args.prompt_mode:
            template_name_save = template_name.replace(' ','-')
            retrieved_dataset_path = os.path.join(retreived_dataset_root, args.dataset_name + "__" + args.dataset_config_name + "__" + template_name_save)
            raw_datasets = load_from_disk(retrieved_dataset_path)
            if "validation" in raw_datasets:
                raw_datasets = raw_datasets["validation"]
            print('loaded dataset with retrieved examples:', retrieved_dataset_path)
        ...
        def preprocess_function_dpr_full_prompt(examples):
            ...
            for i in range(bs):
                ...
                ## sort
                scores, retrieved_examples = ex["scores"], ex['retrieved_examples']
                cands = [
                    (
                        scores[ii], 
                ...
                assert concat_num <= len(cands)
                ex_i = 0
                chosen_key_set = set()
                while len(demonstration_pretokenized) < concat_num and ex_i < len(cands):
                    cand = cands[ex_i]
                    if (cand[3],cand[4]) not in chosen_key_set:
                        # (dataset_name, idx) is not been chosen
                ...
                retrieval_results[template_name].append(
                    {
                        "input":input,
                        "target":target,
                        "retrieved_input":retrieved_examples["inputs_pretokenized"],
                        "retrieved_target":retrieved_examples["targets_pretokenized"],
                        "retrieved_dataset_name":retrieved_examples["dataset_name"],
                        "retrieved_idx":retrieved_examples["idx"],
                ...
                for ii in range(concat_num):
                    prefix = prefix + demonstration_pretokenized[ii]
        ...
        if 'dpr' in args.prompt_mode:
            results['retrieval_database'] = retreived_dataset_root
